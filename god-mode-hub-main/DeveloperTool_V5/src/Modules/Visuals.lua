...
local Visuals = {
    ...
    OriginalCameraMinZoom = nil,
    OriginalCameraMaxZoom = nil,
    FreeCamRoot = nil,
    FreeCamRootAnchored = nil,
    FreeCamHumanoid = nil,
    FreeCamHumanoidAutoRotate = nil,
    FreeCamHumanoidWalkSpeed = nil,
    AtmosphereBackup = {},
    PostEffectsBackup = {},
    FreeCamBackup = nil,
    FreeCamPosition = nil,
...
local function backupCameraZoom(self)
    if self.OriginalCameraMinZoom ~= nil and self.OriginalCameraMaxZoom ~= nil then
        return
    end

    self.OriginalCameraMinZoom = Player.CameraMinZoomDistance
    self.OriginalCameraMaxZoom = Player.CameraMaxZoomDistance
end

local function setCameraZoomUnlocked(self, enabled)
    backupCameraZoom(self)
    self.CameraZoomUnlocked = enabled

    if enabled then
        Player.CameraMinZoomDistance = 0.5
        Player.CameraMaxZoomDistance = 10000
    else
        if self.OriginalCameraMinZoom ~= nil then
            Player.CameraMinZoomDistance = self.OriginalCameraMinZoom
        end
        if self.OriginalCameraMaxZoom ~= nil then
            Player.CameraMaxZoomDistance = self.OriginalCameraMaxZoom
        end
    end
end
...
local function stopFreeCam(self, notifyShared)
    ...
    if self.FreeCamRoot and self.FreeCamRoot.Parent then
        pcall(function()
            self.FreeCamRoot.Anchored = self.FreeCamRootAnchored == true
        end)
    end

    if self.FreeCamHumanoid and self.FreeCamHumanoid.Parent then
        pcall(function()
            self.FreeCamHumanoid.AutoRotate = self.FreeCamHumanoidAutoRotate
            self.FreeCamHumanoid.WalkSpeed = self.FreeCamHumanoidWalkSpeed
        end)
    end

    self.FreeCamRoot = nil
    self.FreeCamRootAnchored = nil
    self.FreeCamHumanoid = nil
    self.FreeCamHumanoidAutoRotate = nil
    self.FreeCamHumanoidWalkSpeed = nil
...
local function startFreeCam(self)
    ...
    self.FreeCamRoot = root
    if root then
        self.FreeCamRootAnchored = root.Anchored
        root.Anchored = true
    end

    local humanoid = Player.Character and Player.Character:FindFirstChildOfClass("Humanoid")
    if humanoid then
        self.FreeCamHumanoid = humanoid
        self.FreeCamHumanoidAutoRotate = humanoid.AutoRotate
        self.FreeCamHumanoidWalkSpeed = humanoid.WalkSpeed
...
function Visuals:Init(context)
    ...
    tab:CreateToggle({
        Name = "解鎖視角縮放限制",
        CurrentValue = false,
        Flag = "DeveloperV5UnlockZoom",
        Callback = function(value)
            setCameraZoomUnlocked(self, value)
        end,
    })
...
function Visuals:Cleanup()
    ...
    self.Alive = false
    stopFreeCam(self, true)
    setCameraZoomUnlocked(self, false)
    stopAtmosphereWatcher(self)
    stopPostEffectWatcher(self)
    ...
    self.OriginalCameraMinZoom = nil
    self.OriginalCameraMaxZoom = nil
    if self.FreeCamRoot and self.FreeCamRoot.Parent then
        pcall(function()
            self.FreeCamRoot.Anchored = self.FreeCamRootAnchored == true
        end)
